...
// 3. Wartość
// (4.) Możemy jeszcze dodać jeden atrybut, którym jest czas życia. Czas życia zmiennej jest zależny od miejsca gdzie
// stworzona jest dana zmienna (globalna i lokalna) - o tym porozmawiamy sobie przy materiale o scope (zakres)


/* CZY ZMIENNA MA TYP? Cze zmienna jest typu string, number, itd?*/
// W wielu językach tak, ale nie w JavaScript. W JavaScript to wartość ma typ a nie zmienna. Wraz z deklaracją nie
// podajemy typu zmiennej. Gdyby JS upodobnić do innych popularnych języków programowania to mogłoby to wyglądać tak:
...
let idUser = 76;
// Przy czym TYPY W JSISTNIEJĄ, ale są nadawane dynamicznie, czyli przez interpreter podczas wykonywania programu
// (mówimy o typowaniu dynamicznym, w przeciwieństwie do typowania statycznego, czyli nadawania typu w kodzie jak w tym
// przykładzie z float). Pamiętajmy jednak, że to nie tworzona zmienna ma typ np.number a właśnie wartość. Zmienna tylko
// przechowuje wartośc.
...
// Tworząć zmienną wybierz const, chyba że zawartosć (wartość przypisana), będzie w trakcie działania programu
// zamienniona inną wartością.
...
const example2 = 200;
// const już na to nie pozwoli (będzie błąd), bo nie można nowej wartości przypisać (pamietajmy, że taka zmiana
// jest tworzeniem nowej wartości)

// example2 = example2 + 1;
// example2++;

// Pamietajmy też, że taki zapis jak ponizej to kopiowana wartość istniejącej zmiennej do nowej zmiennej. Tworzy
// się nowa wartość, to nie jest ta sama wartość, mimo że jest identyczna.
...
// hoisting to przenoszenie przez interpreter na góre kodu (przed jego wykonaniem) deklaracji zmiennych ale nie
// inicjalizacji wartości. Hoisting dotyczy zmiennych deklarowanych za pomocą var, ale nie let i const (to dobrze, bo
// hositing może wprowadzać w błąd).
